...
<head>
  ...
  <title>Dashboard | Reliable Socials</title>
...
<div class="add-fund-menu ml-5 mt-1 hidden space-y-1 text-[13px] font-semibold text-slate-300">
  ...
  <a href="reseller.html" class="block rounded-xl px-4 py-3 hover:bg-slate-800">Become a Reseller</a>
...
<div class="social-boost-menu ml-5 mt-1 hidden space-y-1 text-[13px] font-semibold text-slate-300">
  ...
  <a href="#" data-logout class="mt-2 block rounded-xl px-4 py-3 text-rose-300 hover:bg-slate-800">Logout</a>
    ...
    <main class="px-4 pb-4 pt-0 sm:px-6 sm:pb-6 lg:h-screen lg:overflow-y-auto lg:px-8 lg:pb-8">
      <header class="sticky top-0 z-20 -mx-4 mb-3 flex items-center justify-between border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur sm:-mx-6 sm:px-6 lg:hidden">
        ...
        <p class="text-sm font-extrabold text-slate-700">Reliable Socials</p>
      ...
      <div class="-mx-1 mb-4 grid grid-cols-2 gap-2 px-1 lg:hidden">
        <a href="social-boost.html" class="rounded-xl bg-white px-3 py-2.5 text-center text-xs font-extrabold text-blue-700 shadow-sm">Social Boost</a>
        <a href="data-bundle.html" class="rounded-xl bg-white px-3 py-2.5 text-center text-xs font-extrabold text-cyan-700 shadow-sm">Data Bundle</a>
        <a href="sms-verification.html" class="rounded-xl bg-white px-3 py-2.5 text-center text-xs font-extrabold text-violet-700 shadow-sm">SMS Verification</a>
        <a href="pay-bills.html" class="rounded-xl bg-white px-3 py-2.5 text-center text-xs font-extrabold text-amber-700 shadow-sm">Pay Bills</a>
      </div>
      ...
      <header class="mb-5 hidden items-center justify-between rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm sm:px-6 lg:flex">
        <div><p class="text-xs font-bold uppercase tracking-[0.16em] text-blue-600">Reliable Socials</p><h1 class="mt-1 text-xl font-extrabold sm:text-2xl">User Dashboard</h1></div>
        <div class="flex items-center gap-3"><div class="relative"><button id="notifBtnDesktop" type="button" class="relative grid h-11 w-11 place-items-center rounded-xl border border-slate-200 bg-white text-slate-600 hover:bg-slate-50" aria-label="Open notifications"><svg viewBox="0 0 24 24" class="h-5 w-5" fill="none" aria-hidden="true"><path d="M15 17h5l-1.4-1.4a2 2 0 0 1-.6-1.4V11a6 6 0 1 0-12 0v3.2c0 .5-.2 1-.6 1.4L4 17h5m6 0a3 3 0 1 1-6 0m6 0H9" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg><span class="absolute right-2 top-2 h-2.5 w-2.5 rounded-full bg-rose-500"></span></button><div id="notifPanelDesktop" class="absolute right-0 top-12 hidden w-72 rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_18px_45px_rgba(15,23,42,0.14)]"><p class="px-2 pb-2 text-xs font-black uppercase tracking-[0.14em] text-slate-500">Notifications</p><a href="notifications.html" class="block rounded-xl px-2 py-2 text-sm hover:bg-slate-50"><span class="font-bold text-slate-800">Deposit Received</span><span class="mt-1 block text-xs text-slate-500">Your wallet was credited.</span></a><a href="notifications.html" class="mt-1 block rounded-xl px-2 py-2 text-sm hover:bg-slate-50"><span class="font-bold text-slate-800">Order Completed</span><span class="mt-1 block text-xs text-slate-500">Instagram order delivered.</span></a><a href="notifications.html" class="mt-2 inline-flex text-xs font-bold text-blue-600">View all</a></div></div><a href="add-fund.html" class="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-extrabold text-white hover:bg-blue-700">Deposit</a></div>
      </header>

      <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div><p class="text-sm font-bold text-slate-500">Total Balance</p><h2 data-wallet-balance class="mt-2 text-4xl font-extrabold text-slate-900">&#8358;0</h2><p class="mt-2 text-sm font-bold text-emerald-600">31 Points Available</p></div>
          <button class="rounded-xl border border-blue-100 bg-blue-50 px-4 py-2 text-sm font-extrabold text-blue-700 hover:bg-blue-100">Hide Balance</button>
        </div>
      </section>

      <section class="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <h3 class="text-lg font-extrabold text-slate-900">Quick Services</h3>
        <div class="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          <a href="social-boost.html" class="rounded-2xl border border-slate-200 p-4 hover:border-blue-300 hover:bg-blue-50/40"><p class="text-sm font-black text-blue-700">Social Boost</p><p class="mt-1 text-xs text-slate-500">Grow Instagram, TikTok and more.</p></a>
          <a href="data-bundle.html" class="rounded-2xl border border-slate-200 p-4 hover:border-cyan-300 hover:bg-cyan-50/40"><p class="text-sm font-black text-cyan-700">Data Bundle</p><p class="mt-1 text-xs text-slate-500">Fast airtime and data purchase.</p></a>
          <a href="sms-verification.html" class="rounded-2xl border border-slate-200 p-4 hover:border-violet-300 hover:bg-violet-50/40"><p class="text-sm font-black text-violet-700">SMS Verification</p><p class="mt-1 text-xs text-slate-500">Receive OTP securely in seconds.</p></a>
          <a href="pay-bills.html" class="rounded-2xl border border-slate-200 p-4 hover:border-amber-300 hover:bg-amber-50/40"><p class="text-sm font-black text-amber-700">Pay Bills</p><p class="mt-1 text-xs text-slate-500">Electricity and utility bills.</p></a>
          <a href="sell-crypto.html" class="rounded-2xl border border-slate-200 p-4 hover:border-emerald-300 hover:bg-emerald-50/40"><p class="text-sm font-black text-emerald-700">Sell Crypto</p><p class="mt-1 text-xs text-slate-500">Convert crypto to naira quickly.</p></a>
          <a href="support-ticket.html" class="rounded-2xl border border-slate-200 p-4 hover:border-rose-300 hover:bg-rose-50/40"><p class="text-sm font-black text-rose-700">Support</p><p class="mt-1 text-xs text-slate-500">Get help from our support team.</p></a>
        </div>
      </section>

      <section class="mt-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
          <div><p class="text-sm font-bold text-slate-500">Activity</p><h3 class="mt-1 text-lg font-extrabold text-slate-900">Recent Transactions</h3></div>
          <a href="transactions.html" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-extrabold text-slate-700 hover:bg-slate-50">View All</a>
        </div>
        <div class="overflow-x-auto">
          <table class="min-w-full text-sm">
            <thead><tr class="border-b text-left text-slate-500"><th class="whitespace-nowrap px-3 py-3">Reference</th><th class="whitespace-nowrap px-3 py-3">Type</th><th class="whitespace-nowrap px-3 py-3">Amount</th><th class="whitespace-nowrap px-3 py-3">Status</th></tr></thead>
            <tbody id="dashboardTxRows"></tbody>
          </table>
        </div>
      </section>
    </main>
  ...
  <script>
    ...
    const dashboardTxRows = document.getElementById("dashboardTxRows");
    dashboardTxRows.innerHTML = dashboardHelpers.readTransactions().slice(0, 5).map((tx) => `
      <tr class="border-b border-slate-100 last:border-0">
        <td class="whitespace-nowrap px-3 py-3 font-extrabold text-blue-700">${tx.reference}</td>
        <td class="whitespace-nowrap px-3 py-3 font-extrabold text-slate-900">${tx.type}</td>
        <td class="whitespace-nowrap px-3 py-3 font-extrabold text-slate-900">${dashboardHelpers.money(tx.amount)}</td>
        <td class="whitespace-nowrap px-3 py-3"><span class="${dashboardHelpers.badge(tx.status)}">${tx.status}</span></td>
      </tr>
    `).join("");
  </script>
<script id="add-fund-dropdown-init">
  (function () {
    const groups = [
